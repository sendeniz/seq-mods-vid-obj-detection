...
class Kernel(nn.Module):
    ...
    def forward_state(self, u, state):
        #print("state forward u shape:", u.shape)
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        """See Kernel.forward() for argument documentation."""
        print("---------- SSM Kernel DPLR Forward Section ---------")
        ...
        if state is not None:
            ...
            print(f"_conj(A) shape: {_conj(A).shape}")
            sA = (
                s * _conj(A) # (B H N)
        ...
        k = torch.fft.irfft(k_f, n=discrete_L)  # (B+1, C, H, L) or (0+1, d_inp, d_model, L)
        print(f"k.shape: {k.shape}")
        # # Truncate to target length
        k = k[..., :L]
        print(f"k[..., :L].shape: {k.shape}")
        print(f"k[:-1, :, :, :].shape: {k[:-1, :, :, :].shape}")
        if state is not None:
            k_state = k[:-1, :, :, :]  # (B, C, H, L)
            # or (B, d_input, d_model, L)
        else:
            k_state = None
        k_B = k[-1, :, :, :] # (C H L) or (d_input, d_model, L)
        print(f"K_B shape: {k_B.shape}")
        print(f"Final State: {k_state}")
        return k_B, k_state
...
class FFTConv(nn.Module):
    ...
    def forward(self, x, state=None, rate=1.0, **kwargs): # absorbs return_output and transformer src mask
        ...
        if state is not None:
            ...
            y = y + k_state #
            next_state = self.kernel.forward_state(x)
            print(f"State Update: {next_state}")
        else:
            print("State Is None")
            next_state = None
...
class S4Block(nn.Module):
    ...
    def forward(self, x, state=None, lengths=None, **kwargs): # absorbs return_output and transformer src mask
        """
        ...
        y, state = self.layer(x, state=state, **kwargs)
        print(f"State after FFTCONV: {state}")
