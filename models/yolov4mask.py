import torch
import torch.nn as nn
import torchvision.models as models
from torch.nn.modules.upsampling import Upsample
from torch.utils.checkpoint import checkpoint


class CnnBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=0, stride=1):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels,
            out_channels,
            kernel_size=kernel_size,
            stride=stride,
            padding=padding,
        )
        self.bn = nn.BatchNorm2d(out_channels)
        self.activation = nn.LeakyReLU(0.1)

    def forward(self, x):
        return checkpoint(self._forward, x, use_reentrant=False)

    def _forward(self, x):
        out = self.conv(x)
        out = self.bn(out)
        out = self.activation(out)
        return out

class Upsample(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2, kernel_size=1, padding=0):
        super(Upsample, self).__init__()
        self.upsample = nn.Sequential(
            CnnBlock(
                in_channels=in_channels,
                out_channels=out_channels,
                kernel_size=kernel_size,
                padding=padding,
            ),
            nn.Upsample(scale_factor=scale),
        )

    def forward(self, x):
        return checkpoint(self._forward, x, use_reentrant=False)

    def _forward(self, x):
        return self.upsample(x)


class Downsample(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super(Downsample, self).__init__()
        self.downsample = CnnBlock(
            in_channels=in_channels,
            out_channels=out_channels,
            kernel_size=3,
            stride=2,
            padding=1,
        )

    def forward(self, x):
        return checkpoint(self._forward, x, use_reentrant=False)

    def _forward(self, x):
        return self.downsample(x)


class CnnBlockNoBnActiv(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels, out_channels, kernel_size=3, stride=1, padding=1
        )

    def forward(self, x):
        return checkpoint(self._forward, x, use_reentrant=False)

    def _forward(self, x):
        out = self.conv(x)
        return out


class ScaledPrediction(nn.Module):
    def __init__(self, channels, nclasses, padding=1):
        super().__init__()
        self.nclasses = nclasses
        self.scaled_pred = nn.Sequential(
            CnnBlock(channels, channels * 2, kernel_size=3, padding=1),
            CnnBlock(channels * 2, (nclasses + 5) * 3, kernel_size=1, padding=0),
        )

    def forward(self, x):
        out = self.scaled_pred[0](x)
        out = self.scaled_pred[1](out)
        out = out.reshape(
            x.shape[0], 3, self.nclasses + 5, x.shape[2], x.shape[3]
        ).permute(0, 1, 3, 4, 2)
        return out


class MaskScalePrediction(nn.Module):
    def __init__(self, channels, nclasses, num_masks, padding=1):
        super().__init__()
        self.nclasses = nclasses
        self.num_masks = num_masks
        self.scaled_pred = nn.Sequential(
            CnnBlock(channels, channels * 2, kernel_size=3, padding=1),
            CnnBlock(channels * 2, (nclasses + 5 + num_masks) * 3, kernel_size=1, padding=0),
        )

    def forward(self, x):
        out = self.scaled_pred[0](x)
        out = self.scaled_pred[1](out)
        out = out.reshape(
            x.shape[0], 3, self.nclasses + 5 + self.num_masks, x.shape[2], x.shape[3]
        ).permute(0, 1, 3, 4, 2)
        return out


class SpatialPyramidPooling(nn.Module):
    def __init__(self):
        super().__init__()
        self.pyramid = nn.Sequential(
            nn.MaxPool2d(5, 1, 5 // 2),
            nn.MaxPool2d(9, 1, 9 // 2),
            nn.MaxPool2d(13, 1, 13 // 2),
        )

    def forward(self, x):
        features = [block(x) for block in self.pyramid]
        features = torch.cat([x] + features, dim=1)
        return features


class PathAggregationNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.feature_transform3 = CnnBlock(
            in_channels=64, out_channels=128, kernel_size=1
        )
        self.feature_transform4 = CnnBlock(
            in_channels=160, out_channels=256, kernel_size=1
        )
        self.resample5_4 = Upsample(in_channels=512, out_channels=256)
        self.resample4_3 = Upsample(in_channels=256, out_channels=128)
        self.resample3_4 = Downsample(in_channels=128, out_channels=256)
        self.resample4_5 = Downsample(in_channels=256, out_channels=512)

        self.downstream_conv5 = nn.Sequential(
            CnnBlock(in_channels=2048, out_channels=512, kernel_size=1),
            CnnBlock(in_channels=512, out_channels=1024, kernel_size=3, padding=1),
            CnnBlock(in_channels=1024, out_channels=512, kernel_size=1),
        )

        self.downstream_conv4 = nn.Sequential(
            CnnBlock(in_channels=512, out_channels=256, kernel_size=1),
            CnnBlock(in_channels=256, out_channels=512, kernel_size=3, padding=1),
            CnnBlock(in_channels=512, out_channels=256, kernel_size=1),
            CnnBlock(in_channels=256, out_channels=512, kernel_size=3, padding=1),
            CnnBlock(in_channels=512, out_channels=256, kernel_size=1),
        )
        
        self.downstream_conv3 = nn.Sequential(
            CnnBlock(in_channels=256, out_channels=128, kernel_size=1),
            CnnBlock(in_channels=128, out_channels=256, kernel_size=3, padding=1),
            CnnBlock(in_channels=256, out_channels=128, kernel_size=1),
            CnnBlock(in_channels=128, out_channels=256, kernel_size=3, padding=1),
            CnnBlock(in_channels=256, out_channels=128, kernel_size=1),
        )

        self.upstream_conv4 = nn.Sequential(
            CnnBlock(in_channels=512, out_channels=256, kernel_size=1),
            CnnBlock(in_channels=256, out_channels=512, kernel_size=3, padding=1),
            CnnBlock(in_channels=512, out_channels=256, kernel_size=1),
            CnnBlock(in_channels=256, out_channels=512, kernel_size=3, padding=1),
            CnnBlock(in_channels=512, out_channels=256, kernel_size=1),
        )
        
        self.upstream_conv5 = nn.Sequential(
            CnnBlock(in_channels=1024, out_channels=512, kernel_size=1),
            CnnBlock(in_channels=512, out_channels=256, kernel_size=3, padding=1),
            CnnBlock(in_channels=256, out_channels=512, kernel_size=1),
            CnnBlock(in_channels=512, out_channels=256, kernel_size=3, padding=1),
            CnnBlock(in_channels=256, out_channels=512, kernel_size=1),
        )

    def forward(self, scale1, scale2, scale3):
        return checkpoint(self._forward, scale1, scale2, scale3, use_reentrant=False)

    def _forward(self, scale1, scale2, scale3):
        x1 = self.feature_transform3(scale1)
        x2 = self.feature_transform4(scale2)
        x3 = scale3

        downstream_feature5 = self.downstream_conv5(x3)
        route1 = torch.cat((x2, self.resample5_4(downstream_feature5)), dim=1)
        downstream_feature4 = self.downstream_conv4(route1)
        route2 = torch.cat((x1, self.resample4_3(downstream_feature4)), dim=1)
        downstream_feature3 = self.downstream_conv3(route2)

        route3 = torch.cat(
            (self.resample3_4(downstream_feature3), downstream_feature4), dim=1
        )
        upstream_feature4 = self.upstream_conv4(route3)
        route4 = torch.cat(
            (self.resample4_5(upstream_feature4), downstream_feature5), dim=1
        )
        upstream_feature5 = self.upstream_conv5(route4)

        return downstream_feature3, upstream_feature4, upstream_feature5


class MaskProtoNet(nn.Module):
    def __init__(self, in_channels=512, num_features=256, num_masks=32):
        super().__init__()
        self.num_features = num_features
        self.num_masks = num_masks
        
        # Upsample path
        self.upsample_layers = nn.ModuleList([
            nn.Sequential(
                CnnBlock(512, num_features, kernel_size=1),
                Upsample(in_channels=num_features, out_channels=num_features, scale=2)
            ),
            nn.Sequential(
                CnnBlock(256 + num_features, num_features, kernel_size=1),
                Upsample(in_channels=num_features, out_channels=num_features, scale=2)
            ),
            nn.Sequential(
                CnnBlock(128 + num_features, num_features, kernel_size=1),
                Upsample(in_channels=num_features, out_channels=num_features//2, scale=2)
            ),
            nn.Sequential(
                CnnBlock(num_features//2, num_features//4, kernel_size=1),
                Upsample(in_channels=num_features//4, out_channels=num_features//4, scale=2)
            )
        ])
        
        # Final mask generation
        self.mask_head = nn.Sequential(
            CnnBlock(num_features//4, num_features//8, kernel_size=3, padding=1),
            CnnBlockNoBnActiv(num_features//8, num_masks),
            nn.ReLU(inplace=True)  # Masks should be non-negative
        )

    def forward(self, features):
        # features should be [scale3, scale2, scale1] from PANet
        x = self.upsample_layers[0](features[0])
        x = torch.cat([features[1], x], dim=1)
        
        x = self.upsample_layers[1](x)
        x = torch.cat([features[2], x], dim=1)
        
        x = self.upsample_layers[2](x)
        x = self.upsample_layers[3](x)
        
        masks = self.mask_head(x)
        return masks


class YoloV4_EfficientNet_Mask(nn.Module):
    def __init__(self, nclasses=80, num_masks=32):
        super(YoloV4_EfficientNet_Mask, self).__init__()
        self.nclasses = nclasses
        self.num_masks = num_masks

        # Backbone
        self.efficientnetbackbone = nn.Sequential(
            *list(models.efficientnet_v2_s(weights="DEFAULT").children())[:-2]
        )

        # Adaptation layers
        self.yolov4coadaptation = nn.Sequential(
            CnnBlock(in_channels=1280, out_channels=512, kernel_size=1, padding=0),
            CnnBlock(in_channels=512, out_channels=256, kernel_size=3, padding=1),
            CnnBlock(in_channels=256, out_channels=512, kernel_size=1, padding=0),
        )

        # Neck
        self.yolov4neck = nn.Sequential(
            SpatialPyramidPooling(),
            PathAggregationNet(),
        )

        # Heads with mask coefficients
        self.yolov4head = nn.ModuleList([
            MaskScalePrediction(128, nclasses, num_masks),
            MaskScalePrediction(256, nclasses, num_masks),
            MaskScalePrediction(512, nclasses, num_masks),
        ])
        
        # Mask prototype network
        self.mask_protonet = MaskProtoNet(
            in_channels=512, 
            num_features=256, 
            num_masks=num_masks
        )

    def forward(self, x):
        # Backbone
        backbone_scale1 = checkpoint(
            self.efficientnetbackbone[0][:4], x, use_reentrant=False
        )
        backbone_scale2 = checkpoint(
            self.efficientnetbackbone[0][4:6], backbone_scale1, use_reentrant=False
        )
        backbone_scale3 = checkpoint(
            self.efficientnetbackbone[0][6:], backbone_scale2, use_reentrant=False
        )

        # Neck
        x = self.yolov4coadaptation(backbone_scale3)
        ssp_out = self.yolov4neck[0](x)
        panet_scale1, panet_scale2, panet_scale3 = self.yolov4neck[1](
            backbone_scale1, backbone_scale2, ssp_out
        )
        
        # Detection outputs with mask coefficients
        detections = [
            self.yolov4head[0](panet_scale1),
            self.yolov4head[1](panet_scale2),
            self.yolov4head[2](panet_scale3)
        ]
        
        # Mask prototypes
        masks = self.mask_protonet([panet_scale3, panet_scale2, panet_scale1])
        
        return detections, masks


if __name__ == "__main__":
    img_size = 416
    nclasses = 80
    num_masks = 32
    
    model = YoloV4_EfficientNet_Mask(nclasses=nclasses, num_masks=num_masks)
    x = torch.randn((2, 3, img_size, img_size))
    
    # Forward pass
    detections, masks = model(x)
    
    # Print shapes
    print("Detection outputs:")
    print(f"Scale 1: {detections[0].shape}")  # [batch, 3, img_size//8, img_size//8, 85+num_masks]
    print(f"Scale 2: {detections[1].shape}")  # [batch, 3, img_size//16, img_size//16, 85+num_masks]
    print(f"Scale 3: {detections[2].shape}")  # [batch, 3, img_size//32, img_size//32, 85+num_masks]
    print(f"Mask prototypes: {masks.shape}")  # [batch, num_masks, img_size, img_size]
    
    print("\nModel architecture successfully implemented!")