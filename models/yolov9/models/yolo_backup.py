...
from models.s4nd_model import S4ND_v2
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DualDetect, TripleDetect, DDetect, S4NDDDetect, DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DDetect, S4NDDDetect, Segment, DSegment, Panoptic)):
            s = 256  # 2x min stride
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {Detect, DualDetect, TripleDetect, DDetect, S4NDDDetect, DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic}:
