...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.transpose:
            ...
            # h = w = int(np.sqrt(l))  # assumes square feature map — breaks on non-square (e.g. letterboxed YOLO validation)
            # assert h * w == l        # was commented out to suppress the symptom rather than fix the root cause
            h = int(kwargs.get('h', int(np.sqrt(l))))
            w = int(kwargs.get('w', int(np.sqrt(l))))
