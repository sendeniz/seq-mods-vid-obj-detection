...
def cauchy_naive(v, z, w, conj=True):
    ...
    Device-safe version: ensures all tensors are on the same device
    """
    # Determine target device - prefer CUDA if any tensor is on CUDA
    target_device = v.device
    
    # Check if any tensor is on CUDA and prioritize that
    for tensor in [z, w]:
