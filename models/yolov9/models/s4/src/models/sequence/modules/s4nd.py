...
from models.s4.src.models.sequence import SequenceModule
from models.s4.src.models.sequence.kernels import registry as kernel_registry
from models.s4.src.models.nn import LinearActivation, Activation, DropoutNd
import models.s4.src.utils.train
import models.s4.src.utils as utils


log = models.s4.src.utils.train.get_logger(__name__)
...
class S4ND(SequenceModule):
    ...
    def __init__(
        ...
        refine=False,  # Learnable 1D refine vectors added to K_h, K_w before outer product
        **kernel_args,
    ):
        """
        ...
        # Refine: UR-LSTM-style gated combination of the SSM kernel and a free
        # 1D vector per axis (Gu et al. 2020, "Improving the Gating Mechanism
        # of Recurrent Neural Networks"). Per position t along each axis:
        #   alpha(t) = sigmoid(g(t))
        #   K_eff(t) = alpha(t) * K_ssm(t) + (1 - alpha(t)) * r(t)
        # Initialization (UR-LSTM-U principle, transposed to position-wise gate):
        #   - g is initialized so alpha = sigmoid(g) is uniformly spread across
        #     (lo, hi) over the L positions. This avoids the chicken-and-egg
        #     gradient-flow failure of pure additive refine: at positions where
        #     alpha is small, r receives full gradient; at positions where alpha
        #     is near 1, K_ssm dominates as in base S4ND. Some positions span
        #     the full range so both K_ssm parameters and r parameters have
        #     gradient signal from step 0.
        #   - r is initialized to small random values (not zero) so positions
        #     where alpha approaches 0 don't collapse the kernel output to 0.
        #     std=0.01 keeps the perturbation small while non-degenerate.
        if refine and all(L is not None for L in self.l_max):
            u_lo, u_hi = 0.01, 0.99
            self.r = nn.ParameterList([
                nn.Parameter(0.01 * torch.randn(1, 1, L)) for L in self.l_max
            ])
            self.g = nn.ParameterList([
                nn.Parameter(torch.logit(torch.empty(1, 1, L).uniform_(u_lo, u_hi)))
                for L in self.l_max
            ])
        else:
            self.r = None
    ...
    def forward(self, u, rate=1.0, state=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        # Refine (UR-LSTM-style gated combination): per-axis, per-position the
        # network interpolates between the SSM kernel and a free vector r via
        # a learned gate alpha = sigmoid(g). See __init__ for full rationale.
        # Replaces the previous purely-additive refine.
        if self.r is not None:
            k_refined = []
            for ki, ri, gi in zip(k, self.r, self.g):
                L_k = ki.shape[-1]
                alpha = torch.sigmoid(gi[:, :, :L_k])
                k_refined.append(alpha * ki + (1.0 - alpha) * ri[:, :, :L_k])
            k = k_refined
