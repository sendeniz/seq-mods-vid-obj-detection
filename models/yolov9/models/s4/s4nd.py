...
class S4ND(SequenceModule):
    ...
    def __init__(
        ...
        **kernel_args,
        ...
        # Refine: learnable 1D vectors added to K_h and K_w before outer product.
        # Escapes DPLR exponential decay constraint — r[i] is a free unconstrained vector.
        # Zero init → identical to base S4ND at start of training.
        if refine and all(L is not None for L in self.l_max):
            self.r = nn.ParameterList([
                nn.Parameter(torch.zeros(1, 1, L)) for L in self.l_max
            ])
        else:
            self.r = None
    ...
    def forward(self, u, rate=1.0, state=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        # Refine: add free 1D correction to each kernel before bilateral combination.
        # Gives (K_h + r_h) ⊗ (K_w + r_w) — all 4 terms including cross terms.
        if self.r is not None:
            k = [ki + ri[:, :, :ki.shape[-1]] for ki, ri in zip(k, self.r)]
