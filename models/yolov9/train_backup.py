...
def _safe_half_state_dict(model):
    # .half() silently drops imaginary part of complex params (e.g. S4ND C matrix)
    sd = model.state_dict()
    return {k: v.half() if not v.is_complex() else v for k, v in sd.items()}
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {
                    ...
                    'model': _safe_half_state_dict(deepcopy(de_parallel(model))),
                    'ema': _safe_half_state_dict(deepcopy(ema.ema)),
                    'updates': ema.updates,
