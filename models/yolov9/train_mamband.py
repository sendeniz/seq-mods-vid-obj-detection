...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    amp = check_amp(model)
    # Disable AMP to avoid dtype issues with Mamba SSM kernels
    amp = False
    ...
    # Parameter groups: bias/BN (pg0, zero decay), regular weights (pg1), Mamba weights (pg2)
    mamband_lr = hyp.get('mamband_lr', hyp['lr0'])
    mamband_lrf = hyp.get('mamband_lrf', hyp['lrf'])
    mamband_weight_decay = hyp.get('mamband_weight_decay', hyp['weight_decay'])

    def _is_mamba_param(name):
        return 'mamband' in name.lower() or 's4nd' in name.lower() \
               or 'fusion_box' in name.lower() or 'fusion_cls' in name.lower() \
               or 'box_pred' in name.lower() or 'cls_pred' in name.lower()
    ...
    for k, v in model.named_modules():
        ...
        if isinstance(v, (nn.BatchNorm2d, nn.LayerNorm)):
            ...
            if _is_mamba_param(k):
                pg2.append(v.weight)
            else:
                pg1.append(v.weight)

    # Catch remaining Mamba SSM params (A_log, D, dt_bias etc.) not accessible via .weight
    assigned = {id(p) for p in pg0 + pg1 + pg2}
    for name, param in model.named_parameters():
        if id(param) not in assigned:
            pg2.append(param) if _is_mamba_param(name) else pg0.append(param)

    if opt.optimizer == 'SGD':
        optimizer = torch.optim.SGD(
            [{'params': pg0, 'weight_decay': 0.0},
             {'params': pg1, 'weight_decay': hyp['weight_decay']},
             {'params': pg2, 'lr': mamband_lr, 'weight_decay': mamband_weight_decay}],
    ...
    lf_mamband = lambda x: (1 - x / epochs) * (1.0 - mamband_lrf) + mamband_lrf
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=[lf, lf, lf_mamband])
            ...
            if ni <= nw:
                ...
                for j, x in enumerate(optimizer.param_groups):
                    if j == 0:
                        ...
                        x['lr'] = np.interp(ni, xi, [0.0, x['initial_lr'] * lf_mamband(epoch)])
                    else:
            ...
            if ni - last_opt_step >= accumulate:
                ...
                if epoch == 0 and i == 0:
                    ...
                    for idx, param_group in enumerate(optimizer.param_groups):
                        group_name = ["bias/BN", "regular", "mamband"][idx]
            ...
            if (not nosave) or (final_epoch and not evolve):
                with torch.no_grad():
                    ckpt = {
                        ...
                        'model': de_parallel(model).float().state_dict(),
...
def parse_opt(known=False):
    ...
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-high-mamband.yaml',
...
def main(opt, callbacks=Callbacks()):
    ...
    if not opt.evolve:
        ...
        LOGGER.info(f'Hyperparameter evolution finished {opt.evolve} generations\n'
                    ...
                    f'Usage example: $ python train_mamband.py --hyp {evolve_yaml}')
