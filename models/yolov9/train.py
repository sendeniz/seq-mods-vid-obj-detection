...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    s4nd_lrf = hyp.get('s4nd_lrf', 0.01)
    s4nd_weight_decay = hyp.get('s4nd_weight_decay', 0.00075)
    #s4nd_momentum = hyp.get('s4nd_momentum', hyp['momentum'])

    # Create 3 parameter groups: bias/BN, regular, and s4nd
    pg0, pg1, pg2 = [], [], []

    # S4ND state space parameter names (these go to pg2)
    s4nd_params = ['A_real', 'A_imag', '.B', '.C', '.P', 'inv_dt', '.D', 'output_linear']

    for k, v in model.named_modules():
        if hasattr(v, 'bias') and isinstance(v.bias, nn.Parameter):
            # S4ND LayerNorm bias goes to pg2 (part of S4ND)
            if isinstance(v, nn.LayerNorm) and 's4nd' in k.lower():
                pg2.append(v.bias)
            # Regular BatchNorm2d bias goes to pg0
            elif isinstance(v, nn.BatchNorm2d):
                pg0.append(v.bias)
            # S4ND encoder/decoder bias → pg2 (entire S4ND pipeline)
        ...
        # S4ND LayerNorm weight goes to pg2
        if isinstance(v, nn.LayerNorm) and 's4nd' in k.lower():
            pg2.append(v.weight)
        # Regular BatchNorm2d weight goes to pg0
        elif isinstance(v, nn.BatchNorm2d):
            pg0.append(v.weight)
        elif hasattr(v, 'weight') and isinstance(v.weight, nn.Parameter):
            # S4ND encoder/decoder → pg2 (entire S4ND pipeline)
            if 's4nd' in k.lower() and ('encoder' in k or 'decoder' in k):
                pg2.append(v.weight)
            # Other S4ND weights → pg2
    ...
    # Collect S4ND state space parameters (A_real, B, C, D, P, inv_dt, output_linear)
    for name, param in model.named_parameters():
        if 's4nd' in name.lower() and not any(param is p for p in pg0 + pg1 + pg2):
            if any(kw in name for kw in s4nd_params):
                pg2.append(param)
            else:
                pg1.append(param)
