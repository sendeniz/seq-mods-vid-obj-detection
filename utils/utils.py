...
def non_max_suppression(boxes, iou_threshold, confidence_threshold, use_torch_nms=False):
    """Class-specific Non-maximal suppression on yolo bounding boxes using intersection over union (IoU)
    Arguments:
        boxes (tensor or list): tensor/list of shape (N, 6) in yolo format (class, confidence score, x, y, w, h)
        iou_threshold (float): IoU threshold for NMS
        confidence_threshold (float): minimum valid box confidence threshold
        use_torch_nms (bool): if True, use torchvision.ops.nms (faster), else use custom implementation
    Returns:
        boxes (list): list of boxes after class-specific NMS
    """
    boxes = torch.tensor(boxes) if not isinstance(boxes, torch.Tensor) else boxes
    
    if boxes.size(0) <= 1:
        return boxes.tolist() if boxes.size(0) == 1 else []
    ...
    for cls in classes:
        ...
        if use_torch_nms:
            # Convert from (class, conf, x, y, w, h) to (x1, y1, x2, y2)
            ...
            # Convert boxes
            x1 = cls_boxes[..., 2:3] - (cls_boxes[..., 4:5] / 2)
            y1 = cls_boxes[..., 3:4] - (cls_boxes[..., 5:6] / 2)
            x2 = cls_boxes[..., 2:3] + (cls_boxes[..., 4:5] / 2)
            y2 = cls_boxes[..., 3:4] + (cls_boxes[..., 5:6] / 2)
            scores = cls_boxes[..., 1:2]
    ...
    # Concatenate all kept boxes from all classes
    return torch.cat(keep_boxes, dim=0).tolist()
...
def get_bounding_boxes(rank, loader, model, nms_iou_threshold, anchors, confidence_threshold, use_torch_nms=False):
