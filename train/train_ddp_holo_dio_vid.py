...
import numpy as np
from models.holov4_dio import HoloV4_Dio_EfficentNet
from loss.yolov4loss import YoloV4Loss2, HoloV4Loss2
from utils.dataset_vid import ImageNetVidDataset
from utils.utils import (mean_average_precision, get_bounding_boxes_holo_dio_vid)
from holov4_dio_vid_train_fn import trainholov4_dio_vid_bptt, testholov4_dio_vid
...
def main(rank, world_size, args):
    ...
    writer = SummaryWriter(log_dir="logs/results_holo_dio_vid_bptt/")
    ...
    model = HoloV4_Dio_EfficentNet(maxlength=args.seq_len, nclasses=args.nclasses, gate = args.gate,
    ).to(rank)
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(
        ...
        steps_per_epoch=(
            len(train_loader)
            // (args.target_batch_size // (args.batch_size * args.ngpus))
        )
        + 1,
        anneal_strategy="cos",
    )
    ...
    for run in range(args.nruns):
        for epoch in range(args.nepochs):
            ...
            test_loss, test_class_acc, test_noobj_acc, test_obj_acc = testholov4_dio_vid(
                rank,
                test_loader,
                model,
                loss_f,
            ...
            if (
                ...
                test_pred_boxes, test_true_boxes = get_bounding_boxes_holo_dio_vid(
                    rank,
                    test_loader,
                    model,
                    iou_threshold=0.5,
            ...
            if args.save_model and (
                ...
                if dist.get_rank() == 0:
                    torch.save(
                        ...
                        path_cpt_file + f"holov4_608_vid_run_{run}.cpt",
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hidden_size", type=int, default=512, help="Hidden size used for Hippo Rnn"
    )
    ...
    if parsed_args.gate == "hippolstm":
        print("HoloV4 Dio HippoLstm: initialized with the following parameters:")
    elif parsed_args.gate == "urlstm":
        print("HoloV4 Dio URLstm: initialized with the following parameters:")
    elif parsed_args.gate == "linear":
        print("HoloV4 Dio Linear: initialized with the following parameters:")
    elif parsed_args.gate == "none":
        print("HoloV4 Dio No gateing: initialized with the following parameters:")
