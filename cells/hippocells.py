...
class HippoLegSCell(nn.Module):
    ...
    Hippo class utilizing legs polynomial.
    '''
    def __init__(self, N, gbt_alpha = 0.5, maxlength = 1024, reconst = False, online = False):
        super(HippoLegSCell, self).__init__()
        #print('Online flag:', online)
        if online is False:
            self.N = N
            self.gbt_alpha = gbt_alpha
            self.maxlength = maxlength
            A, self._B = self.get_A_and_B(N = self.N)
            GBTA, GBTB = self.get_stacked_GBT(A = A, B = self._B)
            # can also be removed or changed to torch.float32
            ...
            self.gbt_alpha = gbt_alpha
            self.maxlength = maxlength
            self._A, self._B = self.get_A_and_B(N = self.N)
            self.reconst = reconst
            self.online = online
    ...
    def forward(self, input, c_t = None, t = 0):
        ...
        if self.online is False:
            # c_t = F.linear(c_t.float(), self.A[t]).float() + self.B[t].squeeze(-1) * input
            #print("c_t dtype:", c_t.dtype)
            #print("A_t dtype:", self.A[t].dtype)
            #print("A_t.float dtype:",self.A[t].float().dtype )
            #print("B_t dtype:", self.B[t].squeeze(-1).dtype )
            #print("input dtype", input.dtype)
            c_t = F.linear(c_t.half(), self.A[t].half()) + self.B[t].squeeze(-1) * input
           # print("c_t dtype after:", c_t.dtype)

        elif self.online is True:
            #print("entered online.")
            #print(f"time step: {t}")
            GBTA, GBTB = self.generalized_bilinear_transform(A = self._A, B = self._B, t = t+1, gbt_alpha = self.gbt_alpha)
            GBTA = torch.from_numpy(GBTA).to(device).requires_grad_(False)
            GBTB = torch.from_numpy(GBTB).to(device).requires_grad_(False)
            c_t = F.linear(c_t.float(), GBTA).float() + GBTB.squeeze(-1) * input
...
class GatedHippoCell(nn.Module):
    def __init__(
        ...
        self.hippo_t = HippoLegSCell(
            N=self.hidden_size, gbt_alpha=self.gbt_alpha, maxlength=self.maxlength, online = False
        )
